...
- **Baked dsh plugins** (`internal/toolbox/assets/dsh-plugins/`, wired in `assets/dsh/package.nix`): the image
  ...
  executor subagents → deepseek-v4.1-flash); headless = the same minus dshmarket. dsh-model-router ships
  ...
  published plugin defaults still target v4-flash, so package.nix patches them at build time — executor
  route, `llm-deepseek` catalog (the model picker) and the routing skill's prose all say
  deepseek-v4.1-flash — with `--replace-fail` guards: upstream adopting v4.1 breaks the build, which is
  the signal to drop the patch. NOTE:
...
- **pi packages** (`internal/toolbox/assets/pi-orchestrator/` + `internal/sandbox/pipkgs.go`): pi loads
  ...
  semantics) merges the deepseek-v4.1-flash entry into `~/.pi/agent/models.json` — the baked pi
  release predates the model, and its pi.dev catalog overlay is unreachable under the default egress
  allowlist, so models.json is the only way in; the entry restates the deepseek compat block (pi does
  not inherit a built-in model's compat for a new id) and its context/cost figures mirror
  deepseek-v4-flash — provisional until the pi.dev catalog or a newer pi release carries the model
  officially. Opt out: `piPackages = false` /
