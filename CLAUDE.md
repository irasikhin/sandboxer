...
- **Module / app:** `github.com/irasikhin/sandboxer`; binary `sandboxer`; Go **1.26.7** (the floor is a security floor — CI runs govulncheck on the toolchain go.mod selects, so a reachable stdlib CVE is fixed by RAISING this. The line tracks the nixpkgs devShell's toolchain so `nix build` and CI agree, and 1.25 left upstream support; the history that moved it here: go1.24 carried GO-2026-4601/4602, 1.25.8 became reachable-vulnerable the moment the microVM backend started calling `net.Resolver.LookupHost` — GO-2026-4971, fixed in 1.25.10 — and 1.26 is the floor govulncheck v1.8.0 requires).
...
- **Baked dsh plugins** (`internal/toolbox/assets/dsh-plugins/`, wired in `assets/dsh/package.nix`): the image
  ...
  subagents → DeepSeek V4.1 Flash — upstream's `deepseek-flash`, the id api.deepseek.com serves);
  headless = the same minus dshmarket. dsh-model-router ships
  routing ON by default (its bundle patch) and its rewrite wins over the session's selected model — the
  profile's own cordis.patch.yml layer applies after the plugin layer and can disable the row. Up to
  0.6.3 the published plugin defaulted to v4-pro/v4-flash and the image patched the model ids and prose;
  0.7.0 ships V4.1 Flash itself, so nothing here rewrites models anymore (a future re-pin that changes
  that fails loudly at the string guards if the patch is ever reintroduced). NOTE:
  the plugin still targets the pre-0.1.2 harness API; its package.nix bridges the three symbols the 0.1.2
...
- **pi packages** (`internal/toolbox/assets/pi-orchestrator/` + `internal/sandbox/pipkgs.go`): pi loads
  ...
  semantics) merges the **deepseek-flash** (DeepSeek V4.1 Flash) entry into `~/.pi/agent/models.json` —
  the
  ...
  officially. The id is the one the API serves (measured 2026-09-17: `deepseek-flash` HTTP 200;
  `deepseek-v4.1-flash` HTTP 400 — supported names are `deepseek-flash` and `deepseek-v4-pro`).
  Opt out:
  `piPackages = false` /
