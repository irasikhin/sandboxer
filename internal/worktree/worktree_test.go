...
func TestEnsureFull(t *testing.T) {
	...
	if b := strings.TrimSpace(git(t, dest, "rev-parse", "--abbrev-ref", "HEAD")); b != "sandbox/feat" {
		t.Errorf("branch = %q, want sandbox/feat", b)
	}
	if !IsWorktree(dest) {
...
func TestEnsureReportsProgress(t *testing.T) {
	...
	if got := full.String(); !strings.Contains(got, "full repo") || !strings.Contains(got, "sandbox/f") {
