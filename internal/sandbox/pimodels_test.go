...
func TestEnsurePiModelsWritesFreshModels(t *testing.T) {
	...
	if len(got) != 1 || got[0] != "deepseek-v4.1-flash" {
		t.Errorf("deepseek models = %v, want [deepseek-v4.1-flash]", got)
	}
	if !strings.Contains(progress.String(), "deepseek-v4.1-flash") {
...
func TestEnsurePiModelsMergesExisting(t *testing.T) {
	...
	if got := deepseekModelIDs(t, models); len(got) != 2 || got[0] != "deepseek-chat" || got[1] != "deepseek-v4.1-flash" {
		t.Errorf("deepseek models = %v, want the existing entry kept and deepseek-v4.1-flash appended", got)
	}
...
func TestEnsurePiModelsNeverDuplicates(t *testing.T) {
	existing := `{"providers":{"deepseek":{"models":[{"id":"deepseek-v4.1-flash","name":"my v4.1"}]}}}`
	b := newPiBase(t)
	path := filepath.Join(b.HomeDir("s"), filepath.FromSlash(PiModelsPath))
