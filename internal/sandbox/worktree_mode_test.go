...
func TestNoWorktreeEnvForcesCopy(t *testing.T) {
	repo := gitRepoWithCommit(t)

	// given: worktree mode disabled by env, even though Src is a git repo
	t.Setenv("SANDBOXER_NO_WORKTREE", "1")
	b, err := ResolveBase(repo)
	if err != nil {
		t.Fatal(err)
	}
	if b.RepoRoot != "" {
		t.Fatalf("RepoRoot=%q, want empty (copy mode forced)", b.RepoRoot)
	}

	// when: a sandbox is made → copy path, so the workspace dir exists
	if err := b.MakeSandbox("c", io.Discard); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(b.SandboxDir("c"), "workspace")); err != nil {
		t.Errorf("copy-mode workspace dir missing: %v", err)
	}
	// and: PullDeps is not short-circuited (no profile → nil, but it runs)
	if err := b.PullDeps("c", io.Discard); err != nil {
		t.Errorf("PullDeps copy mode: %v", err)
	}
}

func TestPullDepsGitModeIsNoop(t *testing.T) {
	repo := gitRepoWithCommit(t)
	b, err := ResolveBase(repo)
	if err != nil {
		t.Fatal(err)
	}
	// git mode: PullDeps returns nil without touching anything
	if err := b.PullDeps("any", io.Discard); err != nil {
		t.Errorf("PullDeps git mode = %v, want nil", err)
	}
}
