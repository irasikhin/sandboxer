...
// vars, and only structured fields (srcs vendoring, extraMounts, env) require a
// sandboxer.yaml file. The resolved profile is serialized to JSON (camelCase
// keys) and stored under .sandboxer/_meta/<slug>.profile.json; that JSON is the
// single artifact the container backend and the srcs package read.
package config
